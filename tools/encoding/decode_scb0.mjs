...
const typeRef = (reader, path) => {
  ...
  return [tag, moduleId(reader, `${path}/module`), digestId(reader, `${path}/digest`),
    reader.u32(`${path}/index`)];
};
...
const typeDeclaration = (reader, path) => {
  ...
  const types = [];
  if (tag === 0 || tag === 1) types.push(typeValue(reader,
    `${path}/${tag === 0 ? "target" : "representation"}`));
  if (tag === 2) {
    const fields = table(reader, `${path}/fields`, (p) => reader.name(p),
      (p) => typeValue(reader, `${p}/type`));
    types.push(...fields.map(([, type]) => type));
  }
  if (tag === 3) {
    const cases = table(reader, `${path}/cases`, (p) => reader.u32(p), (p) => ({
    ...
    for (const [, variantCase] of cases) {
...
const parseModulePayload = (reader) => {
  ...
  if (functionOrder.length !== functions.length) fail("0d03",
...
if (process.argv[1] && import.meta.url === new URL(`file://${process.argv[1]}`).href) {
  try {
    const module = decodeModule(fs.readFileSync(process.argv[2]));

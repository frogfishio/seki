...
Status: v0.3 design revision responding to Kiku review; F0 remains open; no
...
Revision v0.3 incorporates the v0.1 and v0.2 corrections and resolves the
remaining SSDC-1 quantifier boundary identified by Kiku:
...
Seki v0.3 adopts proof-carrying invocation: every concrete authority-bearing
decision carries one module/input/result-bound SSDC-1 certificate accepted by
both qualified proof foundations before publication. Seki v0 does not claim a
universal Lean/Rocq bridge.
...
For every qualified proof-carrying invocation, the principal product claim is:
...
The generated Clight-to-Rocq refinement is universal over admitted modules and
inputs. The Lean-to-Rocq equality in the v0 profile is deliberately
invocation-scoped: it holds only when one exact SSDC-1 certificate for that
module, input and result has been accepted by both qualified checkers.
...
Authority publication additionally requires the exact joint SSDC-1 acceptance
receipt. Failure to construct, validate or bind that receipt publishes
nothing.
...
Seki v0.3 pins Lean 4.30.0, Rocq 9.2.0 and CompCert 3.18. It selects the
...
Seki v0 chooses proof-carrying invocation. For each concrete authority-bearing
decision, an untrusted bounded producer may construct an SSDC-1 certificate.
Publication requires the exact same certificate bytes to be accepted by both
qualified checkers against the exact module, semantic profile, input, result
and publication decision.

For that concrete invocation, the portfolio must instantiate:
...
The invocation bridge is the conjunction of both independently checked
soundness theorem instances over the same concrete certificate, not an
assertion that equal bytes imply equal semantics. The portfolio verifier must
authenticate the shared bytes, module, input and result bindings, both checker
acceptances, both theorem instances and both proof-kernel trust reports.
Missing, disagreeing, stale, substituted or ambiguous material rejects before
authority publication.

The publication input contains distinct authenticated objects:
...
The joint receipt binds the exact certificate, typed core, semantic profile,
input, result, publication decision and both qualified checker identities. It
grants no authority by itself; the Profile-C transaction consumes it as a
mandatory premise. It cannot be replaced by two Booleans or by matching result
bytes.

Certificate construction and joint validation are bounded operations. Failure
to produce a certificate accepted by both foundations is a typed proof-gap
result and publishes zero authority. Seki v0 makes no claim that one common
certificate exists for every admitted module and input, and makes no universal
Lean/Rocq evaluator-equality claim.

A future universal-bridge profile may prove:

```text
∀ admitted module input,
  ∃ certificate,
    LeanCheck(module, input, certificate) = accepted ∧
    RocqCheck(module, input, certificate) = accepted
```

That stronger profile must verify one normative constructor projected into
both foundations or otherwise prove common-witness existence. Independent
...
For a qualified proof-carrying invocation, the required source chain is:

```text
Lean Seki result
= jointly accepted SSDC-1 result
= Rocq Seki result
= restricted-C AST result
= embedded CompCert Clight behavior
= verified parse of the exact emitted C bytes
```

The Rocq-Seki-to-Clight portion is proved universally for every admitted
module and input. Only the Lean/Rocq equality is invocation-scoped in v0.
...
→ generated_clight_behavior(m, memory) =
    terminates(
      encoded_result = encode(RocqEval(m, input)),
      final_memory satisfies declared footprint and publication relation)
```

For a qualified proof-carrying invocation, the joint SSDC-1 receipt separately
establishes `RocqEval(m, input) = LeanEval(m, input)`.
...
returns an opaque commit token. Before that token becomes committable, the
exact invocation SSDC-1 certificate must receive both qualified checker
acceptances and one exact joint certificate receipt. The host may publish only
that complete token.
...
5. construct the bounded SSDC-1 invocation certificate;
6. obtain both exact checker acceptances and the joint certificate receipt;
7. construct the authority and publication receipt privately;
8. commit all outputs atomically; and
9. release all private candidates on any failure.
...
Candidate(inputDigest, candidateDigest, jointCertificateReceipt, privateCandidate)
...
- no candidate becomes committable without both qualified checker acceptances
  and the exact joint SSDC-1 receipt;
- a certificate, checker acceptance or joint receipt cannot be substituted
  across module, profile, input, result or publication decision;
...
SSDC-1 proof-carrying invocation construction, the explicit absence of a
universal Lean/Rocq v0 claim, Lean 4.30.0, Rocq 9.2.0, CompCert 3.18 Clight
semantic foundation, API-level host-integrity premises, trust ceiling,
...
For every admitted module and input, prove that generated Clight behavior
equals the Rocq Seki relation under the pinned CompCert behavior and memory
semantics. This universal theorem does not mention or imply the normative Lean
evaluator.

For an exact invocation carrying a valid joint SSDC-1 receipt, separately
prove that the Rocq Seki result equals the normative Lean Seki result. Compose
that invocation-scoped equality with the universal Clight-to-Rocq theorem to
derive generated-C-to-Lean equality only for that jointly certified
invocation.

Without the exact joint certificate receipt, no Lean-equivalence claim and no
authority publication may follow. Qualify both SSDC-1 checkers, their
soundness/completeness theorems and the receipt binding. Prove termination, no
stuck or undefined behavior, exact memory footprint, modeled external calls
only, rejection precedence, resource bounds and zero rejecting publication.
...
- per-invocation joint-certificate receipt and verifier;
- proof-carrying invocation bridge portfolio;
...
## 33. Kiku v0.2 review resolution history
...
Kiku accepted the behavioral Clight, API-confinement and token-linearity paths
but returned a controlled finding against the SSDC-1 bridge. Independent
checker completeness did not prove that both foundations construct one common
certificate for every input. The proposed disposition was:

```text
controlled_finding seki-v0.2 section-17/ssdc-1-cross-foundation-bridge
```

The v0.3 correction adopts Kiku's recommended proof-carrying invocation model
instead of asserting the missing universal premise.

## 34. Kiku v0.3 review resolution

Kiku's third review is incorporated normatively as follows:

1. Every authority-bearing invocation carries one concrete SSDC-1 certificate
   bound to the exact typed core, profile, input, result and publication
   decision.
2. Both qualified checkers must accept those exact certificate bytes.
3. The two soundness theorem instances establish Lean/Rocq equality for that
   invocation through the common certified result.
4. A joint certificate receipt binds both acceptances and is a mandatory
   Profile-C publication premise.
5. Missing, disagreeing, stale, substituted or ambiguous certificate material
   publishes zero authority.
6. Seki v0 claims no universal Lean/Rocq equality and no universal common
   certificate existence.
7. A future stronger profile may add a universal common-witness theorem, but
   independent completeness of two constructors is explicitly insufficient.
8. F4 names the three scopes independently: Clight-to-Rocq is universal,
   Rocq-to-Lean requires the exact joint receipt, and C-to-Lean follows only
   for that jointly certified invocation.
...
- `section-20/certificate-gated-publication`; or
- `section-23/v0-claim-ceiling`; or
- `section-23/F4/refinement-scope`.

## 35. Review questions for SIRCC and Tenkan
